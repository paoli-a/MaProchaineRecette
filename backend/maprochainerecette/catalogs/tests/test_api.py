...
def test_recipes_list_has_correct_fields(recipe):
    ...
    assert recipe_data["ingredients"][0]["ingredient"] == recipe.ingredients.first(
    ).ingredient.name
    assert recipe_data["ingredients"][0]["unit"] == recipe.ingredients.first(
    ).unit.abbreviation
...
def _add_recipe():
    ...
    request_data = {"title": "title recipe",
...
def test_delete_recipe(recipe):
    ...
    request_delete = APIRequestFactory().delete(
        detail_url)
    assert Recipe.objects.count() == 1
    response_delete = RecipeViewSet.as_view(
        {'delete': 'destroy'})(request_delete, pk=recipe.id)
    assert response_delete.status_code == 204
    assert Recipe.objects.count() == 0
